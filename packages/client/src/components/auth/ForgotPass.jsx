...
export default function ForgotPass() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async () => {
      ...
      try {
        ...
        await axiosInstance.post("/users/sendResetPass", qs.stringify(body))
        toast({
  ...
  return (
    <>
      <Link onClick={onOpen} color={'blue.400'} style={{ textDecoration: 'none' }} fontWeight='semibold'>change Password?</Link>
      <Modal
        isOpen={isOpen}
        onClose={onClose}>
        <ModalOverlay />
