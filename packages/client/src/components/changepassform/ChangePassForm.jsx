...
import logogambar from '../../assets/imgs/PHARMACY.png'
...
export default function ChangePassForm() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async () => {
      ...
      try {
        ...
        const res = await axiosInstance.patch(`/users/editPassword/${userSelector.id}`, qs.stringify(body));
        console.log(res);
  ...
  return (
    <>
      ...
      <Stack display='flex' minH='450px' maxW={'500px'} justifyContent={'center'} boxShadow='md' bg='transparent' borderWidth='1px' borderRadius="3" rounded={"15"}>
        ...
        <Stack align={"center"}>
          <Box m={'20px'} width={'300px'}>
            {/* ---------- Password Input ---------- */}
            <FormControl id="repassword" marginTop={'20px'} isInvalid={formik.errors.oldpassword}>
              <InputGroup >
                ...
                />
                <FormLabel className="labelRePass">&nbsp; oldPassword &nbsp;</FormLabel>
                <InputRightAddon>
                  <Icon
                    fontSize="xl"
